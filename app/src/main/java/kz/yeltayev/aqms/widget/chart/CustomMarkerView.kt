package kz.yeltayev.aqms.widget.chart
...
class CustomMarkerView(context: Context, layoutRes: Int) : MarkerView(context, layoutRes) {
    ...
    private val textViewMarkerValue: TextView = findViewById(R.id.tv_marker_amount)

    override fun refreshContent(entry: Entry?, highlight: Highlight?) {
        val theEntry = entry ?: return
        textViewMarkerValue.text = theEntry.y.toString()
    }
}
...
class RoundedBarRenderer(
    ...
    override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
        ...
        while (j < buffer.size()) {
            ...
            drawRoundRect(
                c, buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                buffer.buffer[j + 3]
            )
            drawRoundRect(
                c, buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
